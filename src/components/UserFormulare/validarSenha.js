...
const userSchema = Joi.object({
  ...
  password: Joi.string()
    ...
    .max(30)
    .required()
    .pattern(passwordRegex) // Utiliza a regex para validação da senha
    .messages({
  ...
  confirmpassword: Joi.string()
    .valid(Joi.ref('password')) // Garante que a senha e a confirmação sejam iguais
    .required()
    .messages({
  ...
  .with('password', 'confirmpassword'); // Garante que ambos sejam preenchidos
